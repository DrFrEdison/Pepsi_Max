...
dir( pattern = "Rsource" )
source.file <- print("Rsource_Pepsi_Max_mtx_mop_val_V01.R")
...
matplot(dt$para$wl[[1]]
        ...
        , type = "l", lty = 1, xlab = lambda, ylab = "AU", main = "SL vs Modellspektren"
        , col = "red", add = T)
legend("topright", c(paste0(dt$para$substance[ dt$para$i]), "Ausmischung"), lty = 1, col = c("blue", "red"))

# PLS para ####
dt$model.raw$data$Probe == dt$para$substance[dt$para$i]
dt$para.pls$wlr <- wlr_function(190:280, 190:280, 10); nrow(dt$para.pls$wlr)
dt$para.pls$wlm <- wlr_function_multi(190:280, 190:280, 10); nrow(dt$para.pls$wlm)
...
dt$pls$lin <- linearitaet_filter( linearitaet_prediction =  dt$pls$pred.lin$prediction
                                  , ncomp = dt$para.pls$ncomp
                                  , linearitaet_limit_1 = dt$lin$diff * .925
                                  , linearitaet_limit_2 = dt$lin$diff * 1.075
                                  , R_2 = .8
...
for(i in 1:length(dt$mop$pred)){
  plot(dt$mop$pred[[ i ]]
       , xlab = "", ylab = dt$para$ylab[ dt$para$i ], main = dt$para$txt$loc.line[ i ]
       , ylim = dt$para$SOLL[ dt$para$i] * c(95, 120) / 100, axes = F
       , sub = paste("Bias =", dt$mop$bias[ i ]))
